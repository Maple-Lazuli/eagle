...
class CryptoLayer:
    ...
    def decrypt_message(self, message_json):
        payload = base64.b64decode(message_json['payload']).encode()
        return self.private_key.decrypt(payload, padding.OAEP(
    ...
    def encrypt_message(self, message_json, key_id):
        ...
        payload = base64.encode(payload).decode()
    ...
    def post(self, destination, post_body, key_id):
        ...
        res = r.post(destination, json=json.dumps({"payload": payload}))
        return self.decrypt_message(res.json())